$(document).ready(function(){
	$("#Fantattitude, #closebox").click(function(){
		if($.browser.mozilla)
			if($('#theNiceHider').is(':hidden'))
				$('#theNiceHider').fadeIn('slow');
			
	
		if($("#Fantattitude").css('display') == 'none')
			$('#backwiththetoiletpaper').fadeOut('slow',function(){
				$('#Fantattitude').fadeIn('slow');
			});
		else
			$('#Fantattitude').fadeOut('slow',function(){
				$('#backwiththetoiletpaper').fadeIn('slow');
			});
			
	});
	
	$('#theNiceHider').click(function(){
		$('#theNiceHider').fadeOut('slow');
		$('#theNiceHider').data('showedonce', 1);
	});
